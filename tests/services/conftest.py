...
from tests.utils.scdb import is_scdb_titles_store, is_scdb_numbers_store
...
configs_fixture = (
    ...
    + [(lazy_fixture("test_scdb_path"), conf) for conf in service_configs]
)
...
langs_fixture = (
    ...
    + [
        (lazy_fixture("test_scdb_path"), conf, languages)
        for conf in service_configs[:1]
    ]
)
...
titles_stores_fixture = (
    ...
    + [
        (lazy_fixture("test_pg_path"), conf, is_pg_titles_store)
        for conf in service_configs
    ]
    + [
        (lazy_fixture("test_scdb_path"), conf, is_scdb_titles_store)
        for conf in service_configs
    ]
)
...
numbers_stores_fixture = (
    ...
    + [
        (lazy_fixture("test_scdb_path"), conf, is_scdb_numbers_store)
        for conf in service_configs
    ]
)
...
service_db_path_fixture = [
    ...
    lazy_fixture("scdb_service_db_path"),
]


# For testing CRUD for songs
songs_fixture = (
    [(lazy_fixture("mongo_hymns_service"), song) for song in songs]
    + [(lazy_fixture("pg_hymns_service"), song) for song in songs]
    + [(lazy_fixture("scdb_hymns_service"), song) for song in songs]
)
...
songs_langs_fixture = (
    ...
    + [(lazy_fixture("scdb_hymns_service"), song, languages) for song in songs]
)
...
hymns_service_fixture = [
    ...
    lazy_fixture("scdb_hymns_service"),
]
...
@aio_pytest_fixture
async def scdb_service_db_path(test_scdb_path):
    """the root scdb path for the test service, after setting up configuration"""
    await save_service_config(test_scdb_path, service_configs[0])
    yield test_scdb_path


@aio_pytest_fixture
async def scdb_hymns_service(scdb_service_db_path):
    """the hymns service for use during tests when running on scdb"""
    service = await hymns.initialize(scdb_service_db_path)
    yield service
